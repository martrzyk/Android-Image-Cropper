...
public class CropImageView extends FrameLayout {
    ...
    private ImageMirror mImageMirror;
    //endregion
    ...
    public Bitmap getCroppedImage(int reqWidth, int reqHeight, RequestSizeOptions options) {
        ...
        if (mBitmap != null) {
            ...
            if(mImageMirror == ImageMirror.HORIZONTAL)
               croppedBitmap = BitmapUtils.mirrorImage(croppedBitmap);
        }
    ...
    private void applyImageMatrix(float width, float height, boolean center, boolean animate) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            if (mImageMirror == ImageMirror.DEFAULT || mImageMirror == null)
    ...
    public void setImageMirror(ImageMirror imageMirror) {
        if (imageMirror == null && mImageMirror == null) {
            mImageMirror = ImageMirror.DEFAULT;
        }

        if (mImageMirror != imageMirror) {
            mImageMirror = imageMirror;
            handleCropWindowChanged(false, false);
        }
    }

    public ImageMirror getImageMirror() {
        return mImageMirror;
    }

    //region: Inner class: ImageMirror

    /**
     * Option for showing and using horizontally mirrored picture.
     */
